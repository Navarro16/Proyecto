        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <br>
                    <h1 class="font-semibold text-xl text-gray-800 leading-tight">Editar reserva del local</h1>
                    <div class="mt-6">
                        <form action="{{ url('/administrador/reservaLocal/'. $reservaLocal->id .'/update') }}" class="space-y-1" method="POST" enctype="multipart/form-data">
                            @method('PUT')
                            @csrf                            
                                ...
                                <label for="cedula" class="block text-sm font-medium teext-gray-700 form-lavel">Nombre del cliente </label>
                                    <select name="cedula" id="cedula" class="mt-2 shadow appearence-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline form-control">
                                        @foreach ($clientes as $cliente)
                                            <option value="{{ $cliente->cedula }}"  @if ($cliente->cedula == $reservaLocal->cedula) {{'selected'}} @endif>{{ $cliente->nombre." ".$cliente->apellidos }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <br>
